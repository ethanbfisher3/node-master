...
export function TimeTrialResultScreen({
  ...
  const handleShareScore = async () => {
    try {
      const message = `I just solved ${solvedCount} puzzles in Uncrossed Time Trial (${nodeCountLabel} nodes, ${durationSeconds}s)! Can you beat my score?\n\nOpen in app: ${APP_DEEP_LINK}\nGet it on Google Play: ${PLAY_STORE_URL}`;
      await Share.share({
        message,
        title: "Uncrossed Time Trial Result",
      });
    } catch {
